...
import LeftBarComp from "../../Components/productComp/LeftBarComp"
import RightProduct from "../../Components/productComp/RightProduct"
...
const filterData = (data, type) => {
    const filterdProduct = data.filter((item) => {
        if (type === 'all') {
            return item
        }
...
const rootReducer = (state, action) => {
    if (action.paylode) {
        ...
        updatedState.all.push(action.paylode)
        updatedState.productDetail = filterData(updatedState.all, action.type)
        return { ...updatedState };
    }
...
export const ProductDataProvider = createContext()

function ProductPage() {
    const [state, dispatch] = useReducer(rootReducer, product)
    return (
        <ProductDataProvider.Provider value={{ state, dispatch }}>
